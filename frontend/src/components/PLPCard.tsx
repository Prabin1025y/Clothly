...
const PLPCard = ({ isHovered = false, setHoveredCardId, id }: PLPCardProps) => {
    ...
    return (
        <div className=" rounded-3xl max-w-2xs font-[Inter] cursor-pointer" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            ...
            <div className="relative pt-8 px-8">
                <div className="relative">
                    ...
                    <img
                        ...
                        className={`min-w-[130%] h-80 object-cover absolute left-1/2 -translate-x-1/2 z-1 ${isHovered && "-translate-y-10 animate-bounce animate-subtle-bounce"}  transition-transform duration-300`}
                    />
                </div>
            </div>

            {/* Card Content */}
            <div className={`${isHovered && "bg-gradient-to-b from-accent  to-white "} relative pt-40 pb-4 px-4 text-center shadow-[13px_13px_33px_-15px_#c9a34b] mt-40 rounded-3xl transition-colors duration-300 `}>
                <div className={`${isHovered ? "opacity-100" : "opacity-0"} transition-opacity duration-300 w-[70%] translate-x-9 h-4 bg-transparent shadow-[0px_133px_14px_0px_#000000] rounded-[100%] absolute top-0 `} />
                {/* Product Title */}
